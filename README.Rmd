...
Before importing the file, we can take a look at it in a few ways. `glimpse_rcsv` shows any notes, along with a `head` style sample of the data.
```{r}
...
read.times.with.manipulations <- microbenchmark::microbenchmark(
    readr_read = {
        df.readr <- readr::read_csv( "READMEfiles/readr_write.csv",
            ...
            mutate( posix = as.POSIXct( posix, origin = "1970-01-01T00:00:00Z", tz = "EST" ),
    ...
    dt_fread = { 
        ...
        df.fread.manip[ , posix := as.POSIXct( posix, origin = "1970-01-01T00:00:0Z", tz = "EST" ) ]
